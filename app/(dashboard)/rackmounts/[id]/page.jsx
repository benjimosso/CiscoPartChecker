...
async function getSingleRackmount(id) {
    ...
    const { data, error } = await supabase.from('rackmounts').select('*, cisco(ciscopn)').eq('id', id).single()
    return { data, error }
}
  ...
  return (
    ...
    <div className="flex justify-normal   bg-white w-3/4 h-auto p-6 m-6 rounded-md ">
      ...
      <div className='bg-white m-6  rounded-md'>
        <h1 className="flex text-2xl  text-black font-mono p-6">Compatible with:</h1>
